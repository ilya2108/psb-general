...
* **Высоая доступность** и тестируемость;
* **Низкая связанность**;
...
2. Сервисы доолжны быть доступны по следующим адресам:
   ...
   * ```localhost:TODO``` - сервис обработкки;
   * ```localhost:TODO``` - сервис отмены;
   * ```localhost:TODO``` - сервис журналирования;
...
* [Сервис регистрации](https://github.com/ilya2108/psb-registration). Сервис развернут по адресу ```52.177.241.31:8084```.
* TODO
* TODO
